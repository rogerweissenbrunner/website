...
const navItems = [
  ...
  { href: '/projects', labelKey: 'projects', icon: faCubes },
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col md:flex-row flex-nowrap items-center md:justify-between animate-navbar-enter motion-reduce:animate-none">
        ...
        <div className="w-full py-8 md:w-auto md:flex-1 md:min-w-0">
          <nav className="w-full">
            <ul className="flex flex-row flex-wrap items-center justify-center gap-x-0 gap-y-3 text-center md:justify-end">
              {navItems.map(({ href, labelKey, icon }, index) => {
                const isActive =
                  href === '/'
                    ? pathname === href
                ...
                return (
                  <li key={href} className="flex-1 md:flex-none">
                    <Link
                      href={href}
                      locale={locale}
                      className={`nav-click-bounce nav-hover-glow inline-flex w-full justify-center items-center gap-2 px-2 py-2.5 md:px-3 md:py-2 md:w-auto text-sm transition-colors rounded-none ${
                        index === 0 ? 'rounded-l-md' : ''
                      } ${index === navItems.length - 1 ? 'rounded-r-md' : ''} ${
                        isActive
                          ? 'text-slate-900 dark:text-slate-200 font-medium bg-slate-200/40 dark:bg-slate-700/35'
                          : 'text-slate-700 dark:text-slate-400 hover:bg-slate-200/30 dark:hover:bg-slate-700/25'
                      }`}
                    >
                      <FontAwesomeIcon
                        icon={icon}
                        className="shrink-0 text-slate-800 dark:text-slate-400"
                      />
              ...
              <li className="w-full flex justify-center mt-2 md:w-auto md:mt-0">
                <div className="flex items-center gap-3 md:gap-2">
                  <button
                    type="button"
                    onClick={toggleTheme}
                    className="p-2 md:p-1.5 rounded opacity-90 hover:opacity-100 transition-opacity disabled:opacity-50"
