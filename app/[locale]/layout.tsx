      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){var root=document.documentElement;try{var key='theme';var stored=localStorage.getItem(key);var systemDark=window.matchMedia('(prefers-color-scheme: dark)').matches;var theme=(stored==='light'||stored==='dark')?stored:(systemDark?'dark':'light');if(theme==='dark'){root.classList.add('dark');root.classList.remove('light')}else{root.classList.remove('dark');root.classList.add('light')}}catch(e){}function finishBoot(){root.classList.add('boot-ready')}if(document.readyState==='complete'){finishBoot()}else{window.addEventListener('load',finishBoot,{once:true})}})();`
          }}
        />
      </head>
